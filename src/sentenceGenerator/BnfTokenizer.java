...
public class BnfTokenizer {
	private enum State {
		INITIAL, IN_NONTERMINAL, AFTER_COLON, AFTER_COLON_COLON
	}

	private StreamTokenizer tokenizer;

	/**
	 * Constructs a <code>BnfTokenizer</code> with no text to tokenize; must
	 * call <code>tokenize(String)</code> before asking for tokens.
	 */
	public BnfTokenizer() {
	}

	/**
	 * Constructs a <code>BnfTokenizer</code> with specific initial text to
	 * tokenize.
	 * 
	 * @param text
	...
	/**
	 * Tokenizes BNF definitions and returns each token as a String, one token
	 * per call. <br>
	 * Examples:
	 ...
	 *             If called without setting a string to be tokenized.
	 */
	public String nextToken() throws IllegalStateException {
		...
		if (tokenizer == null) {
			throw new IllegalStateException(
					"BnfTokenizer was not given anything to tokenize!");
		}
